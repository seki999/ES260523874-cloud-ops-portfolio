...
- Vue.js Dashboard: `http://127.0.0.1:5173` で起動し、API 状態、申請一覧、batch 実行、監視ログを見せる画面。
- Java REST API: `/api/health` と `/api/requests` を提供する主要業務 API。
- Python Batch Worker: PENDING 申請を読み取り、AWS 構築処理を mock 実行する。
- Docker MySQL: 申請データを永続化し、Java API と Python worker の連携結果を保存する。
- AWS mock config: 実 credential なしで AWS 設計方針を確認するための設定。

## データ流

1. 利用者が申請を作成または確認する。
2. Java API が申請データを MySQL に保存する。
...
ローカル環境では AWS へ接続しない。設計上は API Gateway を入口にし、ECS で Java API と Python worker を実行、RDS 相当の永続 DB として Docker MySQL を使い、S3 でログと設計成果物を保管、CloudWatch でメトリクスとログ監視、IAM で最小権限を適用する。
