...
Java API が申請管理を担当し、Docker MySQL が申請データを保存し、Python worker が pending 申請を処理する。Dashboard は状態確認と README 画像用に提供する。

## アーキテクチャ方針

ローカルでは Docker Compose と MySQL、設計上は AWS API Gateway / ECS / S3 / RDS / CloudWatch / IAM へ展開できる構成にする。
